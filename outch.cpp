...
struct Stream {
	...
	friend std::ostream& operator<< (std::ostream& os, const Stream&);
...
std::ostream& operator << (std::ostream& os, const Stream& stream) {
	...
	os << "Stream " << int(stream.id) << '\n';
	os << " Accepted: " << stream.e.accepted << " messages: " << '\n';
	os << " System Event: " << stream.e.sys_events << " messages: " << '\n';
	os << " Replaced: " << stream.e.replaced << " messages" << '\n';
	os << " Canceled: " << stream.e.canceled << " messages" << '\n';
	os << " Exceuted: " << stream.e.executed << " messages: " << stream.e.executed_shares << " executed shares" << '\n';
	os << std::flush;
...
struct Decoder<SystemEvent> {
    void operator()(const SystemEvent*, Stream& stream) {
        stream.e.sys_events++;
    }
};
...
struct Decoder<Accepted> {
    void operator()(const Accepted* , Stream& stream) {
        stream.e.accepted++;
    }
};
...
struct Decoder<Replaced> {
    void operator()(const Replaced* , Stream& stream) {
        stream.e.replaced++;
    }
};
...
struct Decoder<Canceled> {
    void operator()(const Canceled* , Stream& stream) {
        stream.e.canceled++;
    }
};
...
struct OutchParser {
	...
	template<typename T>
	[[nodiscard]] static constexpr inline bool isComplete(const uint16_t pl) {
		return pl == sizeof(T);
	}

	template<typename T>
	[[nodiscard]] static const T* as(const char* offset) {
	...
	[[nodiscard]] char msgType(const char* msgoffset) {
...
{
	try {
		FileReader fr("./OUCHLMM2.incoming.packets");
